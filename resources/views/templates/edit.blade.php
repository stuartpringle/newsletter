...
<div class="card p-4">
    <form method="POST" action="{{ cp_route('newsletter.templates.update', $template) }}" class="space-y-4">
        @csrf @method('PUT')

        <div>
            <label class="block text-sm font-medium mb-1" for="name">Name</label>
            <input type="text" id="name" name="name" class="input w-full" value="{{ $template->name }}" required>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label class="block text-sm font-medium mb-1" for="subject">Default Subject</label>
                <input type="text" id="subject" name="subject" class="input w-full" value="{{ $template->subject }}">
            </div>
            <div>
                <label class="block text-sm font-medium mb-1" for="from_name">From Name</label>
                <input type="text" id="from_name" name="from_name" class="input w-full" value="{{ $template->from_name }}">
            </div>
            <div>
                <label class="block text-sm font-medium mb-1" for="from_email">From Email</label>
                <input type="email" id="from_email" name="from_email" class="input w-full" value="{{ $template->from_email }}">
            </div>
        </div>
        ...
        <div>
            ...
            <input id="html" type="hidden" name="html" value="{{ old('html', $template->html) }}">
